...
## refrence
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        ...
        if i > 0:
            sample = es.ask(1).reshape(-1)
            es_param = sample.reshape(n_connections,-1)
            NAS_model.alphas_normal = torch.from_numpy(es_param).float().cuda()
              
        logging.info('epoch %d lr %e', i, lr)
        genotype = NAS_model.genotype()
        logging.info('genotype = %s', genotype)
        
        if not opt.no_train:
            train_acc, train_losses = ces_search(i, train_loader, NAS_model, criterion, optimizer, opt, train_logger, train_batch_logger)
            logging.info('train_acc %f', train_acc)
        if not opt.no_val:
            with torch.no_grad():
                num_valid = i
                if num_valid > pop_size:
                    num_valid = pop_size
                for j in range(num_valid):
                    es_param = es_params[j]
                    print(es_param.shape)
                    es_param = es_param.reshape(n_connections,-1)
                    NAS_model.alphas_normal = torch.from_numpy(es_param).float().cuda()

                    scores[j] = val_epoch(i, val_loader, NAS_model, criterion, opt, val_logger)
                    #scores.append(score)
                    
                    genotype = NAS_model.genotype()
                    logging.info('genotype = %s', genotype)
                    
                    logging.info('valid_acc %f', scores[j])
                
        
        es.tell(es_params, scores)
        old_es_params = deepcopy(es_params)
        #old_scores = deepcopy(scores)