...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3, criterion=nn.CrossEntropyLoss()):
        ...
        self.archi_params = nn.Parameter(torch.randn(10,8))# This is the search weights, 11 is the number of layers
        nn.init.constant(self.archi_params, 1e-3)
    ...
    def forward(self, x):
        ...
        #self._arch_parameters = self._arch_parameters.cuda(x.get_device())
        weights = F.softmax(self.archi_params, dim=-1)
        #print(weights)
...
class Archi_Model(nn.Module):

    def __init__(self, model, criterion=nn.CrossEntropyLoss()):
        super(Archi_Model, self).__init__()

        #self.arch_params = torch.randn(9, device="cuda", requires_grad=True)#1e-3*
        self.arch_params = nn.Parameter(torch.randn(9))
        #self._arch_parameters = [self.arch_params]
        self._criterion = criterion
    ...
    def new():
        print('I am too lazy to immplete.')
    ...
    def arch_parameters(self):
        return self._arch_parameters # should be a iterable one
        
    def forward(self, x):
        return model(x, self.arch_params)
